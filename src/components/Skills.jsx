...
export default function Skills() {
    ...
    return (
        <main className="px-60 dark:bg-[#252128]" data-theme={darkMode ? "dark" : "light"}>
            <h2 className="font-semibold text-5xl text-gray-800 dark:text-[#AEBCCF]">{data[language].skillsSection.title}</h2>
                ...
                <div className="skill-item flex justify-between">
                    {data[language].skillsSection.skills.map((skill, index) => (
                        <div key={index} className="w-[300px] h-[104px]">
                            ...
                            <p className="text-gray-500 font-normal mt-8 text leading-5 dark:text-white">{skill.info}</p>
                        </div>
                    ))}
                </div>
            </section>
